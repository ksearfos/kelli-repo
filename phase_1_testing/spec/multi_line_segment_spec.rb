...
describe "HL7" do
  ...
  describe "multi-line Segment" do
    ...
    context "iteration" do    
      ...
      it "happens naturally by line" do
        count = 0
        @segment.each{ |f|
          count += 1
        }
    ...
    context "undefined method" do         
      it "will treat segment as a String" do
        @segment.gsub( '*', '|' ).should == $seg_2_line.gsub( '*', '|' ) 
      end
      
      it "will treat segment as an Array" do
        full_str = $seg_str + "|" + $seg_str2
        @segment.sort.should == full_str.split("|").sort
      end
